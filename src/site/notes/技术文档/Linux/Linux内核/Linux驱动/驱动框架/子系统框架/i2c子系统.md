...
主要工作:
1. 驱动注册到平台总线上, 参考[[技术文档/Linux/Linux内核/源码结构/include/linux/platform_device.h#platform_driver_register宏\|platform_device.h#platform_driver_register宏]]
2. 初始化 `i2c_adapter` 结构体变量，然后设置 `i2c_algorithm` 中的 `master_xfer` 函数
3. 完成以后通过 `i2c_add_numbered_adapter`
或`i2c_add_adapter`这两个函数向`I2C` 子系统注册设置好的`i2c_adapter`
4. 总线驱动的删除通过`i2c_del_adapter`
## I2C client, I2C设备驱动
...
-  `i2c_set_clientdata`用来设置私有数据
-  `i2c_get_clientdata`用来获取私有数据
- `i2c_register_driver/i2c_add_driver`注册驱动
- `i2c_del_driver`注销驱动
